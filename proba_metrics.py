...
def calc_mean_binary_cross_entropy_from_probas(ytrue_N, yproba1_N):
    ...
    log2_1_minus_p = np.log2([1 - p for p in yproba1_N])
    log2_p = np.log2(yproba1_N)

    total = 0.0
    for i in range(N):
        term = -1 * ytrue_N[i] * log2_p[i] - (1 - ytrue_N[i]) * log2_1_minus_p[i]
        total = total + term
...
def calc_mean_binary_cross_entropy_from_scores(ytrue_N, scores_N):
    ...
    `numpy.numpy.logaddexp` or `scipy.special.logsumexp`.
    ...
    flipped_scores_N = yflippedsign_N * scores_N # TODO fix me: flip(y_n) s_n
    ...
    denom = np.log([2])[0]
    for i in range(N):
        num = scipy.special.logsumexp([0, flipped_scores_N[i]])
        term = num / denom
        total += term
